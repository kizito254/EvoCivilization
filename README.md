...
Added an optional Phase 5 performance layer (enabled with `--phase5`) that includes:

- **Optimized stepping mode** (`_step_fast`) that iterates alive agents and compacts dead entries.
- **Long-run stability helper** with periodic resource regeneration under scale.
- **Scaling benchmark harness** for 1k/5k/10k agent scenarios with JSON report export.

### Run with Phase 5

```bash
python3 run_phase1.py --phase5 --agents 10000 --ticks 20
```

### Run benchmark report

```bash
python3 run_phase1.py --benchmark --phase5 --ticks 20 --benchmark-json artifacts/phase5_benchmark.json
```
