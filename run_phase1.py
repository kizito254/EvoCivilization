...
def main() -> None:
    ...
    parser.add_argument("--phase5", action="store_true", help="Enable Phase 5 performance/scaling mode")
    parser.add_argument("--benchmark", action="store_true", help="Run Phase 5 scaling benchmark scenarios")
    ...
    parser.add_argument("--benchmark-json", type=str, default="artifacts/phase5_benchmark.json")
    args = parser.parse_args()

    if args.benchmark:
        report = run_scaling_benchmark(
            seed=args.seed,
            ticks=max(5, args.ticks),
            enable_phase2=args.phase2 or args.phase3 or args.phase5,
            enable_phase3=args.phase3 or args.phase5,
            enable_phase5=True,
        )
        out = write_benchmark_json(report, args.benchmark_json)
        print("Phase 5 benchmark complete:")
        for case in report.cases:
            print(f"  agents={case.agents} ticks={case.ticks} seconds={case.seconds:.3f} final_alive={case.final_alive}")
